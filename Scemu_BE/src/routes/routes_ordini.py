...
def get_ordini(
    ...
    ordini = cr.get_tutti_ordini(skip, limit)
...
def aggiorna_stato(ordine_id: int, nuovo_stato: md.StatoOrdine = Query(...)):
    ordine = cr.get_ordine_by_id(ordine_id)           
    ...
    if nuovo_stato.value not in stati_validi:
        ...
        if not stati_validi:
            messaggio = (f"Lo stato '{stato_corrente}' è terminale e non consente ulteriori transizioni. "
                        f"Non è possibile cambiare lo stato dell'ordine.")
        else:
            messaggio = (f"Transizione non valida: {stato_corrente} → {nuovo_stato.value}. "
                        f"Transizioni valide da '{stato_corrente}': {', '.join(stati_validi)}")
        raise HTTPException(400, messaggio)
